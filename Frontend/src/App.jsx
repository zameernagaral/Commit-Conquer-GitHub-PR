...
import { Check, Ban, Lock, Clock, ChevronsLeftRightEllipsis, GitPullRequest, Users } from 'lucide-react';
...
export default function App() {
      ...
      if (!isLocked) {
        handler({
          ...
          pr: { 
            ...
            timestamp: Date.now(), 
            mergedCount: 0,
            members: [membersList[Math.floor(Math.random() * 4)], membersList[Math.floor(Math.random() * 4)]] 
          }
        });
      }
    }, 25000);
  ...
  const handleApprove = async (prId, comment) => {
    ...
    setToasts(prev => [
      ...prev, 
      { id: toastId, text: <><Check size={16} className="inline mr-2" /> PR Approved</> }
    ]);
  ...
  prs.forEach(pr => {
    if (!allTeamsMap[pr.team]) {
      ...
      allTeamsMap[pr.team].mergedCount = Math.max(allTeamsMap[pr.team].mergedCount, pr.mergedCount || 0);
      if (!allTeamsMap[pr.team].members?.length && pr.members) {
        allTeamsMap[pr.team].members = pr.members;
      }
    }
  });
  const sortedTeams = Object.values(allTeamsMap).sort((a, b) => b.mergedCount - a.mergedCount);
  ...
  const filteredTeams = sortedTeams.filter(team => team.team.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="app-container">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <button 
            ...
            style={{
              ...
              background: currentView === 'prs' ? 'var(--bg-card)' : 'transparent', 
              border: `1px solid ${currentView === 'prs' ? 'var(--border-light)' : 'transparent'}`,
              borderRadius: '12px', cursor: 'pointer', fontWeight: '600', 
              color: currentView === 'prs' ? 'var(--text-main)' : 'var(--text-muted)',
              boxShadow: currentView === 'prs' ? '0 2px 8px rgba(0,0,0,0.04)' : 'none',
              transition: 'all 0.2s', width: '100%', textAlign: 'left'
            }}
          >
            <GitPullRequest size={18} /> Submissions
          </button>
          ...
          <button 
            ...
            style={{
              display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px',
              background: currentView === 'teams' ? 'var(--bg-card)' : 'transparent', 
              border: `1px solid ${currentView === 'teams' ? 'var(--border-light)' : 'transparent'}`,
              borderRadius: '12px', cursor: 'pointer', fontWeight: '600', 
              color: currentView === 'teams' ? 'var(--text-main)' : 'var(--text-muted)',
              boxShadow: currentView === 'teams' ? '0 2px 8px rgba(0,0,0,0.04)' : 'none',
              transition: 'all 0.2s', width: '100%', textAlign: 'left'
            }}
          >
      ...
      <main className="main-content">
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          <input 
            type="text" 
            placeholder={currentView === 'prs' ? "Search submissions..." : "Search teams..."} 
            style={{ background: '#fff', border: '1px solid var(--border-light)', padding: '10px 16px', borderRadius: '12px', width: '300px', fontSize: '13px', outline: 'none' }} 
            value={searchQuery} 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            <div style={{ fontSize: '13px', fontWeight: '500', color: isLocked ? 'var(--status-rejected-text)' : 'var(--text-main)' }}>
              {isLocked ? (
                ...
                <><Clock size={16} className="inline mr-1" /> Submissions close in: {formatTime(timeLeft)}</>
              )}
            </div>
            
            <div style={{ fontSize: '13px', fontWeight: '500', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '6px' }}>
             <ChevronsLeftRightEllipsis size={16}/> {teamsOnline} Teams Online
            </div>
            <NotificationBell 
              items={notifications} 
              onClear={() => setNotifications([])} 
              onClickItem={(pr) => { setSelected(pr); setCurrentView('prs'); }} 
            />
          </div>
        </header>
        ...
        {currentView === 'prs' ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <h2 style={{ fontSize: '20px', marginBottom: '16px', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Users size={24} /> Registered Teams
            </h2>
            {filteredTeams.length === 0 ? (
              <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '13px', border: '1px solid var(--border-light)', borderRadius: '12px', background: 'var(--bg-card)' }}>No teams found.</div>
            ) : (
              filteredTeams.map((team, idx) => (
                <div key={idx} style={{ 
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between', 
                  padding: '20px 24px', background: 'var(--bg-card)', borderRadius: '12px', 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: '700', fontSize: '16px', color: 'var(--text-main)' }}>{team.team}</div>
                      <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                        {team.members && team.members.length > 0 ? team.members.map((initials, i) => (
                          <span key={i} style={{ fontSize: '11px', background: 'var(--bg-main)', padding: '4px 10px', borderRadius: '6px', color: 'var(--text-muted)', fontWeight: '600', border: '1px solid var(--border-light)' }}>
                            {initials}
                          </span>
                        )) : (
                          <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>No members listed</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: '800', fontSize: '20px', color: 'var(--text-main)' }}>
                      {team.mergedCount}
                    </div>
